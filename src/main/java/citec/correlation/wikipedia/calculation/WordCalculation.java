...
import citec.correlation.wikipedia.dic.lexicon.ResultDetail;
...
public class WordCalculation implements TextAnalyzer {
    ...
    private  Map<String, List<ResultDetail>> wordEntities = new TreeMap<String,List<ResultDetail>>();
  ...
  private  String entityResultToString(List<EntityResults> entityResults) {
        ...
        for (EntityResults entities : entityResults) {
            ...
            for (WordResult wordResults : entities.getDistributions()) {
                ...
                if (wordEntities.containsKey(key)) {
                    List<ResultDetail> propertyObjects = wordEntities.get(key);
                    ResultDetail entityInfo = new ResultDetail(wordResults.getPosTag(),entities.getProperty(), entities.getKB(), wordResults.getMultipleValue(), wordResults.getProbabilities());
                    propertyObjects.add(entityInfo);
                    this.wordEntities.put(key, propertyObjects);
                } else {
                    List<ResultDetail> propertyObjects = new ArrayList<ResultDetail>();
                    ResultDetail entityInfo = new ResultDetail(wordResults.getPosTag(),entities.getProperty(), entities.getKB(), wordResults.getMultipleValue(), wordResults.getProbabilities());
                    propertyObjects.add(entityInfo);
                    this.wordEntities.put(key, propertyObjects);
                }
    ...
    public Map<String, List<ResultDetail>> getWordEntities() {
        return wordEntities;
    }
